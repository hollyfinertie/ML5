...
# Test performance
results1 = predict(model1, test_alc, type = 'prob')
results_prob1 = ifelse(results1 > 0.5,1,0)

outcome1 = (as.numeric(test_alc$alc_outcome))

testProbs =  data.frame(obs = test_alc$alc_outcome,
                        pred.logit=results_prob1)

missclass1 = mean(results_prob1 != outcome1, na.rm = T)
print(paste('Accuracy Model 1',1 - missclass1))

calPlotData = calibration(obs ~ pred.logit.1, data = testProbs, cuts=5)
