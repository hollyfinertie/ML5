...
model.lasso.test.pred = model.lasso.train.final %>% predict(x.test) %>% as.vector()

data.frame(RMSE = RMSE(model.lasso.test.pred, life.exp.test), RSQ = R2(model.lasso.test.pred, life.exp.test))

# ELASTIC
model.elastic.cv = cv.glmnet(x.train, life.exp.train, alpha = 0.5)
plot(model.elastic.cv)

model.elastic.train.final = glmnet(x.train, life.exp.train, alpha = 0.5, lambda = model.elastic.cv$lambda.min)

model.elastic.test.pred = model.elastic.train.final %>% predict(x.test) %>% as.vector()

data.frame(RMSE = RMSE(model.elastic.test.pred, life.exp.test), RSQ = R2(model.elastic.test.pred, life.exp.test))
```
...
# Model coefficients
coef(model.4$finalModel, model.4$bestTune$lambda)

# Make predictions

pred4 = model.4 %>% predict(x.test)

# Model prediction performance
data.frame(
  RMSE = RMSE(pred4, test.data$life_exp),
  Rsquare = R2(pred4, test.data$life_exp))
```
